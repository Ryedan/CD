...
export class Attachments {
    ...
    parse(attachments, embeds, files) {
        ...
        const [embed] = embeds;
        return attachments
            .reduce((parsedAttachments, { type, photo, video, link, doc, audio, poll, album, textlive, market, podcast }) => {
            ...
            switch (type) {
                ...
                case PODCAST: {
                    ...
                    const prefix = audio ?
                        ...
                        :
                            '🎙️ Подкаст';
                    parsedAttachments.push(hyperlink(`${prefix}: ${artist} - ${title}`, `${LINK_PREFIX}${type}${owner_id}_${id}`));
                    break;
                }
                case POLL: {
                    const { owner_id, id, question } = poll;
                    parsedAttachments.push(hyperlink(`📊 Опрос: ${question}`, `${LINK_PREFIX}${this.#generateAttachmentContext(poll)}?w=${type}${owner_id}_${id}`));
                    break;
                }
                case ALBUM: {
                    const { owner_id, id, title } = album;
                    parsedAttachments.push(hyperlink(`🖼️ Альбом: ${title}`, `${LINK_PREFIX}${type}${owner_id}_${id}`));
                    break;
                }
                case MARKET: {
                    const { owner_id, id, title } = market;
                    parsedAttachments.push(hyperlink(`🛍️ Товар: ${title}`, `${LINK_PREFIX}${type}${owner_id}?w=product${owner_id}_${id}`));
                    break;
                }
                case MARKET_ALBUM: {
                    const { owner_id, id, title } = market;
                    parsedAttachments.push(hyperlink(`🛍️ Подборка товаров: ${title}`, `${LINK_PREFIX}${type}${owner_id}?section=${ALBUM}_${id}`));
                    break;
                }
                case TEXTLIVE: {
                    const { textlive_id, title } = textlive;
                    parsedAttachments.push(hyperlink(`📣 Репортаж: ${title}`, `${LINK_PREFIX}${type}${textlive_id}`));
                    break;
                }
            }
            return parsedAttachments;
