import { Handler, Storage } from './modules/index.js';
// @ts-ignore
import config from '../config.json' assert { type: 'json' };
const { clusters } = config;
console.log('[VK2Discord] Запущен.');
const handlers = await Promise.all(clusters
    .map((cluster, index) => (new Handler({
    ...cluster,
    index: index + 1
})
    .init())));
...
uniqueHandlers.forEach(async ([storage, groupIds]) => {
    ...
    const outdatedKeys = storageKeys.reduce((keys, key) => {
